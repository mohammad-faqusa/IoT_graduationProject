...
async function espSetup(id, plist, socket) {
  try {
    ...
    const { stdout3, stderr3 } = await execPromise(
      `mpremote connect COM3 fs cp ${__dirname}/espFiles/main.py :main.py`
    );
    console.log("Output:", stdout3);
    if (stderr3)
      socket.emit("processSetup", {
        status: "processing",
        data: `Error Output:, ${stderr3}`,
      });
    socket.emit("processSetup", {
      status: "processing",
      data: `copied main file`,
    });

    const { stdout4, stderr4 } = await execPromise(
      `mpremote connect COM3 fs cp ${__dirname}/espFiles/boot.py :boot.py`
    );
    console.log("Output:", stdout4);
    if (stderr4)
      socket.emit("processSetup", {
        status: "processing",
        data: `Error Output:, ${stderr4}`,
      });
    socket.emit("processSetup", {
      status: "processing",
      data: `copied boot file`,
    });
    ...
    setTimeout(async () => {
      ...
      if (stderr5)
        socket.emit("processSetup", {
          status: "processing",
          data: `Error Output:, ${stderr5}`,
        });
    }, 2000);

    socket.emit("processSetup", {
      status: "finished",
      data: "the setup is done successfully!",
    });
  } catch (error) {
